...
class Reservas extends Controller
{
  ...
  {
    $data = $this->model->editarReservas($id);
  ...
  {$dataint=$this->model->getInstitucion();
    $datares= $this->model->editarReservas($id_MaxReserva);
    ...
    $pdf->Cell(0, 10, 'utf8_decode'("Hora de Ingreso : " . $datares['hora_inicio']." a ".$datares['hora_fin']), 0, 1);

    $pdf->Cell(0, 10, 'Total: ' .$datares['precio']. ' Bs.' , 0, 1);
    $pdf->Cell(0, 10, '-----------------------------------------------', 0, 1, 'C');
    $pdf->Ln(2); // Salto de línea
